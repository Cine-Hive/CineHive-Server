...
@Service
public class AnimationGenreService {
    private final Map<Integer, String> genreMap = new HashMap<>();
    private final WebClient webClient;
    private final String apiKey;
    private final ObjectMapper objectMapper;

    public AnimationGenreService(WebClient.Builder webClientBuilder, @Value("${tmdb.api.key}") String apiKey, ObjectMapper objectMapper) {
    ...
    private void loadGenres() {
        String response = webClient.get()
                .uri("https://api.themoviedb.org/3/genre/movie/list?api_key=" + apiKey + "&language=ko")
                .retrieve()
