<<<<<<< HEAD:CineHive/src/main/java/com/example/CineHive/domain/oauth/controller/OAuth2Controller.java
package com.example.CineHive.domain.oauth.controller;

import com.example.CineHive.domain.auth.entity.ProviderType;
import com.example.CineHive.domain.oauth.service.OAuth2Service;
=======
package com.example.CineHive.domain.oauth;

import com.example.CineHive.domain.auth.ProviderType;
>>>>>>> parent of 49bd7c6b ([Ref]: 도메인 패키지 구조 정리):CineHive/src/main/java/com/example/CineHive/domain/oauth/OAuth2Controller.java
...
public class OAuth2Controller {
    ...
    @Operation(summary = "소셜 로그인 콜백 처리 (웹 전용)",
            ...
            **[서버의 역할]**
            ...
            3.  **성공 시, `User-Agent` 헤더 정보를 바탕으로 사용자의 로그인 이력(브라우저 정보 등)이 기록됩니다.**
            4.  모든 처리가 성공하면, 서버는 CineHive 서비스에서 사용할 수 있는 자체 **JWT(JSON Web Token)**를 생성하여 응답 본문에 담아 반환합니다.
            
            **[요청 헤더]**
            - **`User-Agent`**: 로그인 이력 기록을 위해 **필수**로 포함되어야 합니다. (웹 브라우저는 이 헤더를 자동으로 전송하므로, 웹 클라이언트 개발자는 별도로 신경 쓸 필요 없습니다.)
    ...
    @Operation(summary = "소셜 로그인 (앱 전용)",
            ...
            **[요청 헤더 (Request Headers)]**
            - **`User-Agent`**: 로그인 이력 기록을 위해 **필수**로 포함해야 합니다. (예: `CineHive-iOS/1.0.2`, `okhttp/4.9.1`)
