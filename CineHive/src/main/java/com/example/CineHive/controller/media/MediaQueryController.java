...
@Tag(name = "Media Query Controller", description = "영화, TV 시리즈 등 미디어 콘텐츠 탐색 API")
@Validated
@RestController
...
public class MediaQueryController {
    ...
    @Operation(summary = "미디어 통합 검색",
            description = "키워드로 영화와 TV 시리즈를 통합 검색합니다. 결과는 페이징되어 반환됩니다.")
    @GetMapping("/search")
    ...
    @Operation(summary = "메인 화면용 차트 요약 조회",
            description = "메인 화면에 표시될 여러 차트의 요약 목록(각 차트별 상위 10개)을 한번에 조회합니다. 반환되는 차트의 종류와 순서는 관리자 API를 통해 동적으로 변경될 수 있습니다.")
    ...
    @Operation(summary = "큐레이션 차트 상세 조회",
            description = "서버에 미리 정의된 특별 큐레이션 차트의 전체 목록을 페이지별로 조회합니다.")
    ...
    @Operation(summary = "플랫폼별 TV 시리즈 차트 조회",
            description = "특정 플랫폼의 TV 시리즈 목록을 인기순으로 조회합니다. 사용 가능한 플랫폼 이름 목록은 `GET /api/v1/media/meta/filters`를 통해 얻을 수 있습니다.")
    ...
    @Operation(summary = "필터 메타데이터 조회",
            description = "클라이언트에서 검색/탐색 필터링 UI를 동적으로 구성하는 데 필요한 모든 데이터를 제공합니다. (장르, 플랫폼, 정렬 옵션 등)")
