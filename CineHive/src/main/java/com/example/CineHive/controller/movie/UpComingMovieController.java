...
public class UpComingMovieController {
    ...
    @PostMapping("/movies/upcoming")
    public ResponseEntity<?> saveUpComingMovies() {
    ...
    @ResponseBody
    public ResponseEntity<List<VideoDto>> getUpComingMoviesList() {
        Pageable pageable = PageRequest.of(0, 22); // 첫 번째 페이지에서 22개 가져오기
        List<VideoDto> upComingMovies = upComingMovieService.getUpComingMovies(pageable);
