...
/**
 * 게시글 '좋아요'(BoardLike) 엔티티에 대한 데이터 접근을 처리하는 JpaRepository입니다.
 */
@Repository
public interface LikeRepository extends JpaRepository<PostDislike, Long> {
     ...
     */
    boolean existsByMemberAndBoard(User user, Post post);
     ...
     */
    Optional<PostDislike> findByMemberAndBoard(User user, Post post);
    ...
    /**
     * 특정 회원이 누른 모든 '좋아요'를 데이터베이스에서 삭제합니다.
     * 이 메서드는 주로 회원 탈퇴 시 연관된 데이터를 정리할 때 사용됩니다.
     *
     * @param email 삭제할 '좋아요'의 소유자 이메일
     */
    @Modifying
    @Query("DELETE FROM PostDislike l WHERE l.member.email = :email")
