...
public class  AnimationController {
    ...
    @ResponseBody
    public ResponseEntity<VideoDto> getAnimationById(@PathVariable Long id) {
        Optional<Animation> animationOptional = animationRepository.findById(id);
        if (animationOptional.isPresent()) {
            return ResponseEntity.ok(convertToVideoDto(animationOptional.get()));
        } else {
    ...
    public List<VideoDto> getMovies(){
        return animationRepository.findAll().stream()
    ...
    @ResponseBody
    public ResponseEntity<List<VideoDto>> getSimilarAnimations(@PathVariable Long id) {
        List<Animation> similarAnimations = similarAnimationService.getSimilarAnimations(id);
        List<VideoDto> videoDtos = similarAnimations.stream()
                .map(this::convertToVideoDto)
                .collect(Collectors.toList());
        return ResponseEntity.ok(videoDtos);
    }
    
    // Animation 객체를 VideoDto로 변환하는 헬퍼 메서드
    private VideoDto convertToVideoDto(Animation animation) {
        List<String> genreNames = animation.getGenres().stream()
                .map(genre -> genre.getName())
                .collect(Collectors.toList());
                
        return new VideoDto(
                animation.getId(),
                animation.getPosterPath(),
                animation.getName(),
                animation.getReleaseDate() != null ? animation.getReleaseDate().toString() : null,
                genreNames
        );
    }
}
